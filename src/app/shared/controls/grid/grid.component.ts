...
import { GridFilter, Pager, Button_Type } from './common_model';
...
export class GridComponent {
  ...
  @Input() extraButtons: Button_Type[];
  ...
  @Output('ExtraButtonClick') ExtraButtonClick: EventEmitter<any> =
    new EventEmitter<any>();
  @ViewChild('condition') condition: ElementRef;
  @ViewChild('container') container: ElementRef;

  fileFolderPath: string = environment.Base_File_Path;
  ///////////////// Start => Currency Symble ////////////////////////
  countryid: any;
  currencyname: string = '';
   ///////////////// Start => Currency Symble ////////////////////////
  ...
  selectedRow: any;
  selectedAll: any;
  isShowfiltermenu: boolean = false;
  top: number;
  left: number;
  ...
  constructor(public service: SystemService, public eRef: ElementRef, public changeDetector: ChangeDetectorRef, public router: Router, private http: HttpClient,
  ...
  onDocumentClick(event: Event) {
    if (
      this.currentTarget &&
      this.currentTarget.nativeElement != event.target
    ) {
  ...
  ngOnChanges() {
    ...
    if (this.service.Account.Is_Agent) {
      this.pagesize = this.service.Account.PageSize
        ? this.service.Account.PageSize
        : this.pagesize;
    } else {
      this.pagesize = this.service.Account.PageSize_Client
        ? this.service.Account.PageSize_Client
        : this.pagesize;
    }
  ...
  async ngOnInit() {
    ...
    // Load company profile and currency
    await this.loadCompanyProfile();

    // Set default currency for gridfilter columns
    if (this.gridfilter) {
      this.gridfilter.forEach((col) => {
        if (col.Is_Currency && !col.CurrencySymbol) {
          col.CurrencySymbol = this.currencyname; // API value or default
        }
      });
    }
  ...
  async getCountryById(id: number) {
    ...
    if (response.isSuccess) {
      this.currencyname = response.data.currency; // e.g., '₹' ya 'INR'
      console.log('Currency from API:', this.currencyname);
    } else {
      this.currencyname = '₹'; // fallback
    }
  }
  ...
  changeSorting(columnName: string, Type: string, isFilter: boolean = false) {
    ...
    if (Type == 'number' || Type == 'int' || columnName.indexOf('_Unix') > -1) {
      isNumber = true;
    }
    this.SortType = Type;
    ...
    if (!isFilter) {
      this.SortDirection =
        this.SortDirection.toLowerCase() === 'asc' ? 'desc' : 'asc';
    }
    var isNumeric: boolean =
      (columnName && columnName.indexOf('Amount') != -1) || isNumber;
    ...
    if (!is_Date) {
      this.records.sort(
        sortFunc(columnName, this.SortDirection === 'desc', primer)
      );
    } else {
      this.records.sort((a: any, b: any) => {
        let rev = this.SortDirection === 'desc';
        let aCol = a[columnName];
        let bCol = b[columnName];
  ...
  ColumnFilter(
    ...
    window.setTimeout(() => {
      ...
      this.left =
        elemRect.left -
        (cRect.left + (condRect.width / 2 - elemRect.width / 2));
      ...
      for (let i = 0; i < this.gridfilter.length; i++) {
        if (
          this.gridfilter[i].ColumnName ==
          this.gridfilter[this.ColumnIndex].ColumnName

        ) {
          let cnd =
            this.gridfilter[i].Condition == 'no'
              ? ''
              : this.gridfilter[i].Condition;
          if (
            document
              .getElementsByClassName('filter_list_ul')[0]
  ...
  FilterConditionClick(event: any, condition: string) {
    ...
    if (condition == '') {
      this.gridfilter[this.ColumnIndex].Value = '';
    }
  ...
  FilterOnTextboxEnter(
    ...
    if (ColumnValue == null || ColumnValue == '') {
      ...
      this.gridfilter[this.ColumnIndex].Condition = 'contain';
    } else if (ColumnType == 'number' || ColumnType == 'int') {
      this.gridfilter[this.ColumnIndex].Condition = 'equal';
    }
  ...
  Filter() {
    ...
    this.gridfilter.forEach((d) => {
      if (
        ...
        if (d.Type == 'string') {
          if (d.Condition == 'equal') {
            filterArr = filterArr.filter((item) => {
              return (
                item[d.ColumnName] &&
                item[d.ColumnName].toString().toLowerCase() ==
                d.Value.toLowerCase()
              );
            });
          ...
          if (d.Condition == 'equal') {
            filterArr = filterArr.filter((item) => {
              return (
                item[d.ColumnName] &&
                this.service.Date_Format(item[d.ColumnName], 'yyyy-MM-dd') ==
                val
              );
            });
          } else if (d.Condition == 'notequal') {
            ...
            filterArr = filterArr.filter((item) => {
              return (
                item[d.ColumnName] &&
                this.service.Date_Format(item[d.ColumnName], 'yyyy-MM-dd') > val
              );
            });
          ...
          if (d.Condition == 'equal') {
            filterArr = filterArr.filter((item) => {
              return (
                item[d.ColumnName] &&
                parseFloat(item[d.ColumnName]) == parseFloat(d.Value)
              );
            });
          } else if (d.Condition == 'notequal') {
            filterArr = filterArr.filter((item) => {
        ...
        if (d.Condition == 'isnull') {
          ...
          filterArr = filterArr.filter((item) => {
            return (
              item[d.ColumnName] == null || item[d.ColumnName].toString() == ''
            );
          });
    ...
    if (this.SearchText && filterArr && filterArr.length > 0) {
      ...
      let keys = this.gridfilter
        .filter((d) => d.Is_Visible == true)
        .map((d) => {
          return d.ColumnName;
        });
  ...
  RowClick(item: any, event: any, index: number) {
    if (
      ...
      //add empty condition for not select row when click on subject
  ...
  ButtonClick(item, action) {
    this.ExtraButtonClick.emit({ item: item, action: action });
  }
  ...
  PageSizeChange(pagesize: any) {
    this.pagesize = this.pager.pageSize = Number(pagesize.target.value);
    this.setPage(1);
  }
  setPage(event: any) {
    if (event.page < 1 || event.page > this.pager.totalPages) {
      return;
    }
    ...
    this.pager = this.getPager(this.totalitems, event.page, this.pagesize);
    this.pagedItems = this.records.slice(
  ...
  async Export(ftype: string) {
    try {
      ...
      if (items.length > 0) {
        ...
        let columns = this.gridfilter
          .filter((d) => d.Is_Visible && d.ColumnName != 'HasAttachment')
          .map((val) => {
            return val;
          });
        ...
        return this.http
          .post(this.baseUrl + 'core/Export_Data', obj, {
  ...
  getTotal(amount, columnName) {
    // console.log(columnName)
    return amount.reduce(
      (accumulator, current) => accumulator + current[columnName],
      0
    );
    // return amount.reduce((acc, {amount}) => acc += +(amount || 0), 0);
  }
