...
@Component({
  selector: 'app-printreceiptvoucher',
  templateUrl: './printreceiptvoucher.component.html',
  styleUrl: './printreceiptvoucher.component.css',
})
export class PrintreceiptvoucherComponent {
  voucherId: number;
  receiptVoucherData: IReceiptVoucher;
  ...
  async getVoucherById(id: number) {
    let res = await this.genericSErvice.ExecuteAPI_Get<IReceiptVoucher>(
      `ReceiptVoucher/GetReceiptVoucherById?id=${id}`
    );

    if (res.isSuccess) {
      this.receiptVoucherData = res.data;
    } else {
