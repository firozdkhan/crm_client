...
@Component({
  selector: 'app-master-deduction',
  templateUrl: './master-deduction.component.html',
  styleUrl: './master-deduction.component.css',
})
export class MasterDeductionComponent implements OnInit {
  @ViewChild('goUp', { static: true }) contentPage: ElementRef;
  MasterAllownaceform: FormGroup;
  MasterAllownacelist: IDeduction[] = [];
  MasterAllownace: IDeduction;
  action: string = 'new';
  ...
  constructor(
    ...
    this.gridFilter.push(<GridFilter>{
      DisplayText: 'Deduction Name',
      ColumnName: 'miscDeductionName',
      Type: 'string',
      Is_Sort: true,
      Is_Visible: true,
    });
  ...
  createCategoryform() {
    this.MasterAllownaceform = this.fb.group({
      id: [0],
      miscDeductionId: ['', Validators.required],
  ...
  async bindAttendance() {
    let res = await this.genericService.ExecuteAPI_Get<IResponse>(
      'MasterDeduction/GetAllMasterDeduction'
    );
    if (res.isSuccess) {
  ...
  async onSubmit(form: FormGroup) {
    ...
    if (this.buttonText === 'Submit') {
      res = await this.genericService.ExecuteAPI_Post<IResponse>(
        'MasterDeduction/AddMasterDeduction',
        formData
      );
    } else {
      res = await this.genericService.ExecuteAPI_Put<IResponse>(
        'MasterDeduction/UpdateMasterDeduction',
        formData
      );
    }
    if (res.isSuccess) {
    ...
    if (result.isConfirmed) {
      try {
        let res = await this.genericService.ExecuteAPI_Delete<IResponse>(
          `MasterDeduction/DeleteMasterDeduction?id=${id}`
        );
        if (res.isSuccess) {
          Swal.fire(
            'Deleted!',
